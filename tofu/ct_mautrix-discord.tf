resource "proxmox_virtual_environment_container" "mautrix-discord" {
  node_name     = "aetherium"
  vm_id         = 118
  description   = "Mautrix-Discord bridge for Matrix (NixOS)"
  start_on_boot = true
  started       = true
  unprivileged  = true
  template      = false
  tags          = ["nixos", "matrix", "bridge"]
...
resource "null_resource" "deploy_flake_mautrix-discord" {
  triggers = {
    container_id = proxmox_virtual_environment_container.mautrix-discord.id
  }
  depends_on = [proxmox_virtual_environment_container.mautrix-discord]
