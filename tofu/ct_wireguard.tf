resource "proxmox_virtual_environment_container" "wireguard" {
  node_name    = "aetherium"
  vm_id        = 113
  description  = "Wireguard VPN server (NixOS)"
  start_on_boot = true
  started      = true
  unprivileged  = true
  template      = false
  tags         = ["nixos", "wireguard"]
...
resource "null_resource" "deploy_flake_wireguard" {
  triggers = {
    container_id = proxmox_virtual_environment_container.wireguard.id
  }
    ...
    command = <<-EOT
      ...
      while [ $i -lt 30 ]; do
        ...
        if ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand="$PROXY" \
              ...
              root@10.0.0.113 "echo ready" 2>/dev/null; then
      ...
      tar cz --owner=0 --group=0 -C ${path.root}/.. . | \
        ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand="$PROXY" \
          -i ${var.ssh_private_key_path} \
          root@10.0.0.113 \
          "tar xz -C /etc/nixos && nix --extra-experimental-features 'nix-command flakes' profile install nixpkgs#git && nixos-rebuild switch --flake /etc/nixos/nixos#wireguard --show-trace && nix-collect-garbage --delete-old"
    EOT
  }
}
